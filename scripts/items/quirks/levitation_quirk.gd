...
## Средняя подъёмная сила в долях веса. Меньше единицы намеренно: она же
## задаёт, с какой скоростью вещь возвращается вниз, оказавшись в воздухе.
## При 0.95 перевес вниз составлял пять процентов веса, сопротивление среды
## съедало и его, и вылетевшая из кузова вещь просто зависала. При 0.8
## она снижается примерно на 33 px/с — медленно, но зримо.
@export_range(0.0, 2.0, 0.05) var lift_base: float = 0.8
## Размах колебания. Отвечает за всход: 0.45 при базе 0.8 даёт разброс
## от 0.35 (оседает) до 1.25 (всходит примерно на 39 px).
@export_range(0.0, 1.0, 0.05) var lift_amplitude: float = 0.45
...
@export var rise_drag: float = 5.0
