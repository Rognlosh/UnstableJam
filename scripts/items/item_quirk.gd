...
## Нужно ли держать тело бодрствующим. Физика усыпляет вещь, которая долго
## почти не двигалась, а у спящей _integrate_forces не вызывается — значит,
## свойство, работающее каждый кадр, замолкает и разбудить себя уже не может.
## Левитации сон противопоказан; взрыву, который срабатывает один раз,
## он безразличен, и тогда флаг стоит снять.
@export var keeps_awake: bool = true
