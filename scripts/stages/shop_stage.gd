...
## Смена языка на лету. Godot сам переводит заново только тот текст, в котором
## лежит ключ; всё, что скрипт собрал через tr() и записал строкой, для движка
## уже готовая фраза. Витрину поэтому пересобираем руками — иначе игрок,
## переключивший язык из паузы, остаётся на прежнем до конца дня.
##
## Проверка is_node_ready() обязательна: с 4.3 это уведомление приходит ещё
## и при входе узла в дерево, то есть до _ready(), когда собирать нечего.
